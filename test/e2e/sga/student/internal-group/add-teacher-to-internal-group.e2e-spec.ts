...
const path = `/internal-group/${GetAllInternalGroupsE2eSeed.internalGroupId}/add-teacher`;

describe('/internal-group/{id}/add-teacher (PUT)', () => {
  ...
  it('should add a teacher to the internal group', async () => {
    ...
    const edaeUser = await edaeUserRepository.findOne({
      where: { id: GetAllInternalGroupsE2eSeed.edaeUserId },
    });
    ...
    expect(internalGroup!.teachers.map((teacher) => teacher.id)).toContain(
      edaeUser!.id,
    );
  });
