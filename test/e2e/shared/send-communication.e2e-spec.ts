...
const path = `/communication/${EditCommunicationE2eSeed.communicationId}/send`;
const worngPath = `/communication/${EditCommunicationE2eSeed.sentCommunicationId}/send`;
...
describe('/communication/{id}/send (put)', () => {
  ...
  it('should throw CommunicationNotFoundException', async () => {
    const response = await supertest(httpServer)
      .put('/communication/4c347b78-4f79-4cce-8d83-b48d17d6c52e/send')
      .auth(superAdminAccessToken, { type: 'bearer' })
      .send(requestBody)
      .expect(404);
  ...
  it('should send a communication', async () => {
    ...
    expect(communication?.sendByEmail).toEqual(true);
  });
