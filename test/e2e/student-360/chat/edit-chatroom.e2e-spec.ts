...
const path = `/student-360/chatroom/${EditChatroomSeed.chatroomId}`;
...
describe('/student-360/chatroom/:id (PUT)', () => {
  ...
  beforeAll(async () => {
    ...
    studentToken = await loginStudent(
      ...
      EditChatroomSeed.studentPassword,
    );
  });
  ...
  it('should return a 404 chatroom not found', async () => {
    const response = await supertest(httpServer)
      .put('/student-360/chatroom/63e9649a-5b20-4476-b8b6-4bd2357f05c8')
      .auth(studentToken, { type: 'bearer' })
      .send({
